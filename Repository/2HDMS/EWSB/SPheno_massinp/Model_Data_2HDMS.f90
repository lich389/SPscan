...
Complex(dp) :: Lam1ppIN,Lam4pIN,Lam6pIN,Lam7pIN,Lam5pIN,Lam2ppIN,Lam6IN,Lam5IN,Lam7IN,               & 
& Lam3pIN,Lam1IN,Lam4IN,Lam3IN,Lam2IN,Lam1pIN,Lam2pIN,Lam3ppIN,YuIN(3,3),epsUIN(3,3),    & 
& YdIN(3,3),YeIN(3,3),epsDIN(3,3),epsEIN(3,3),MUS1IN,MU11IN,MU12IN,MU21IN,               & 
& MU22IN,MUS2IN,MSpIN,M12IN,M112IN,M222IN,MS2IN

Real(dp) :: g1MZ,g2MZ,g3MZ

Complex(dp) :: Lam1ppMZ,Lam4pMZ,Lam6pMZ,Lam7pMZ,Lam5pMZ,Lam2ppMZ,Lam6MZ,Lam5MZ,Lam7MZ,               & 
& Lam3pMZ,Lam1MZ,Lam4MZ,Lam3MZ,Lam2MZ,Lam1pMZ,Lam2pMZ,Lam3ppMZ,YuMZ(3,3),epsUMZ(3,3),    & 
& YdMZ(3,3),YeMZ(3,3),epsDMZ(3,3),epsEMZ(3,3),MUS1MZ,MU11MZ,MU12MZ,MU21MZ,               & 
& MU22MZ,MUS2MZ,MSpMZ,M12MZ,M112MZ,M222MZ,MS2MZ

Real(dp) :: g1GUT,g2GUT,g3GUT

Complex(dp) :: Lam1ppGUT,Lam4pGUT,Lam6pGUT,Lam7pGUT,Lam5pGUT,Lam2ppGUT,Lam6GUT,Lam5GUT,              & 
& Lam7GUT,Lam3pGUT,Lam1GUT,Lam4GUT,Lam3GUT,Lam2GUT,Lam1pGUT,Lam2pGUT,Lam3ppGUT,          & 
& YuGUT(3,3),epsUGUT(3,3),YdGUT(3,3),YeGUT(3,3),epsDGUT(3,3),epsEGUT(3,3),               & 
& MUS1GUT,MU11GUT,MU12GUT,MU21GUT,MU22GUT,MUS2GUT,MSpGUT,M12GUT,M112GUT,M222GUT,MS2GUT
...
Logical, save :: InputValueforLam1pp =.False. 
...
Logical, save :: InputValueforLam2pp =.False. 
...
Logical, save :: InputValueforLam3pp =.False. 
...
Logical, save :: InputValueforMSp =.False. 
...
Complex(dp) :: Lambda3pInput
Complex(dp) :: Lambda4pInput
Complex(dp) :: Lambda5pInput
Complex(dp) :: Lambda6pInput
Complex(dp) :: Lambda7pInput
Complex(dp) :: Lambda1ppInput
Complex(dp) :: Lambda2ppInput
Complex(dp) :: Lambda3ppInput
Complex(dp) :: MUS1Input
Complex(dp) :: MUS2Input
...
Lambda3pInput=(0._dp,0._dp)
Lambda4pInput=(0._dp,0._dp)
Lambda5pInput=(0._dp,0._dp)
Lambda6pInput=(0._dp,0._dp)
Lambda7pInput=(0._dp,0._dp)
Lambda1ppInput=(0._dp,0._dp)
Lambda2ppInput=(0._dp,0._dp)
Lambda3ppInput=(0._dp,0._dp)
MUS1Input=(0._dp,0._dp)
MUS2Input=(0._dp,0._dp)
...
& vS,g1,g2,g3,Lam1pp,Lam4p,Lam6p,Lam7p,Lam5p,Lam2pp,Lam6,Lam5,Lam7,Lam3p,Lam1,           & 
& Lam4,Lam3,Lam2,Lam1p,Lam2p,Lam3pp,Yu,epsU,Yd,Ye,epsD,epsE,MUS1,MU11,MU12,              & 
& MU21,MU22,MUS2,MSp,M12,M112,M222,MS2,MZsuffix)
...
If (MZsuffix) Then 
  ...
  YeMZ = (YeSM*(vd - vu*zEInput))/vSM 
  YdMZ = (YdSM*(vd - vu*zDInput))/vSM 
  YuMZ = (YuSM*(vd - vu*zUInput))/vSM 
  epsEMZ = (YeSM*(vu + vd*zEInput))/vSM 
  epsDMZ = (YdSM*(vu + vd*zDInput))/vSM 
  epsUMZ = (YuSM*(vu + vd*zUInput))/vSM 
  g1MZ = g1SM 
  g2MZ = g2SM 
  g3MZ = g3SM 
Else 
  ...
  Ye = (YeSM*(vd - vu*zEInput))/vSM 
  Yd = (YdSM*(vd - vu*zDInput))/vSM 
  Yu = (YuSM*(vd - vu*zUInput))/vSM 
  epsE = (YeSM*(vu + vd*zEInput))/vSM 
  epsD = (YdSM*(vu + vd*zDInput))/vSM 
  epsU = (YuSM*(vu + vd*zUInput))/vSM 
  g1 = g1SM 
  g2 = g2SM 
  g3 = g3SM 
End if 
End Subroutine SetMatchingConditions 
End Module Model_Data_2HDMS